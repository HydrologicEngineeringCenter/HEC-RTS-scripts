# -------------------------------------------------------------------
# Extract USGS Config Builder
#
# Creates/edits a .config file that stores:
#   - path to a .usgs file
#   - path to a TimeSeries.dss file
#
# Intended for use with Extract_USGS script
# -------------------------------------------------------------------
...
CONFIG_VERSION = "1"
DEFAULT_CONFIG_NAME = "extract_usgs.config"
...
# Default .usgs location: <projectDir>\shared\source\<Watershed>.usgs
DEFAULT_USGS_PATH = os.path.join(PROJ_DIR, "shared", "source", WATERSHED_NAME + ".usgs")
# Default DSS location: go up two directories from <projectDir>, then: database\USGS\<Watershed>_TimeSeries.dss
DEFAULT_DSS_PATH = os.path.join(CWMS_HOME, "database", "USGS_Data", WATERSHED_NAME + "_TimeSeries.dss")
...
def save_config(config_path, usgs_file, dss_file):
	usgs_file = _abs_norm(usgs_file)
	dss_file = _abs_norm(dss_file)

	f = None
	try:
		f = open(config_path, "w")
		f.write("# USGS paths config\n")
...
def choose_file(parent, title, extensions, desc, initial_path=None, directories_only=False):
	fc = JFileChooser()
...
class USGSConfigDialog(JDialog):
	def __init__(self, parent, config_path):
		JDialog.__init__(self, parent, "USGS Paths Configuration", True)
		self.config_path = config_path

		loaded = load_config(config_path)
		
		usgs_default = loaded.get("usgs_file", "").strip() or DEFAULT_USGS_PATH
		dss_default  = loaded.get("dss_file", "").strip()  or DEFAULT_DSS_PATH

		self.usgsField = JTextField(usgs_default, 45)
	...
	def _build_ui(self, parent):
		...
		gbc.gridy = 1
		gbc.gridx = 0
		panel.add(JLabel("Select .usgs file:"), gbc)

		gbc.gridx = 1
		panel.add(self.usgsField, gbc)

		btnBrowseUSGS = JButton("Browse...")
		gbc.gridx = 2
		panel.add(btnBrowseUSGS, gbc)
		...
		# Wiring actions
		class BrowseUSGS(ActionListener):
			def actionPerformed(self, event):
				initial = self_outer.usgsField.getText().strip()
				if not initial:
				...
				path = choose_file(
					...
					"Select .usgs file",
					["usgs"],
					"USGS Control File (*.usgs)",
					initial_path=initial,
					directories_only=False
				)
				if path:
					self_outer.usgsField.setText(path)
		...
		class SaveAction(ActionListener):
			def actionPerformed(self, event):
				usgs_path = _abs_norm(self_outer.usgsField.getText())
				dss_path  = _abs_norm(self_outer.dssField.getText())

				# Basic validation
				if (not usgs_path) or (not usgs_path.lower().endswith(".usgs")):
					JOptionPane.showMessageDialog(self_outer, "Please select a valid .usgs file.", "Validation", JOptionPane.WARNING_MESSAGE)
					return
				if not os.path.isfile(usgs_path):
					JOptionPane.showMessageDialog(self_outer, "The .usgs file does not exist:\n{0}".format(usgs_path), "Validation", JOptionPane.WARNING_MESSAGE)
					return
				...
				try:
					save_config(self_outer.config_path, usgs_path, dss_path)
				except Exception as e:
		...
		self_outer = self
		btnBrowseUSGS.addActionListener(BrowseUSGS())
...
def run_dialog(config_path, parent=None):
	dlg = USGSConfigDialog(parent, config_path)
	dlg.setVisible(True)
